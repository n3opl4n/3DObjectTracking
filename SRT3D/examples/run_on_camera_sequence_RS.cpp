...
#include <srt3d/realsense_camera.h>
...
int main() {
  ...
  const std::filesystem::path model_directory{
      "/home/fotis/3DObjectTracking-master/SRT3D/examples/digestive/"};

  constexpr bool kSaveViewerImage = false;
  const std::filesystem::path viewer_save_directory{
      "/home/fotis/3DObjectTracking-master/SRT3D/examples/"};
  ...
  // Set up camera
  auto camera_ptr{std::make_shared<srt3d::RealSenseColorCamera>("azure_kinect")};
  ...
  const std::filesystem::path body1_geometry_path{"/home/fotis/3DObjectTracking-master/SRT3D/examples/digestive/digestive.obj"};
  ...
  body1_ptr->set_maximum_body_diameter(.25);
  renderer_geometry_ptr->AddBody(body1_ptr);

  // Set up model body 1
  auto body1_model_ptr{std::make_shared<srt3d::Model>(
      "body1_model", body1_ptr, model_directory, "digestive_model.bin")};
